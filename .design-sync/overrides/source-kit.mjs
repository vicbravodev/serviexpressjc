// forked from design-sync lib/source-kit.mjs — exclude legacy ui/toast.tsx +
// ui/toaster.tsx from the synth barrel: their `Toaster` star-export collides
// with ui/sonner.tsx's and esbuild drops the ambiguous name from the bundle.
...
// Legacy shadcn toast system, unused by the app; sonner.tsx is the shipped
// Toaster. Keeping both in the barrel makes `Toaster` ambiguous and undefined.
const BARREL_EXCLUDE_RX = /components\/ui\/(toast|toaster)\.tsx$/;
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !BARREL_EXCLUDE_RX.test(slash(p)));
